...
// degrees to radian wrapper`
export const degToRad = (amt) => {
  return THREE.MathUtils.degToRad(amt);
};

const mergeMesh = (target) => {
  //THREE.SceneUtils.createMesh()
};

export const fileStats = (file) => {
  let index = file.lastIndexOf("/");
  let res = file.substr(index + 1);
  const dot = res.lastIndexOf(".");
  res = res.substr(0, dot);
  return { name: res, ext: res.substr(dot) };
};
