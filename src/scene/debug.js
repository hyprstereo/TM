...
import { IOCScene } from "./ioc.js";


export const TomiPositions = {
    pos: []
}
export const ActionProps = (ctx, face, sceneManager, actionTriggered = undefined) => ({

    running: false,
    greeting: () => {
        face.action(3, 3, 0, 1000);
        ctx.play(0);
        if (actionTriggered) actionTriggered('greeting');
    },
    wink: () => {
        face.action(8, face._eyeRight, -1, 250);
        if (actionTriggered) actionTriggered('wink');
    },
    blink: () => {
        face.action(9, 9, -1, 100);
        if (actionTriggered) actionTriggered('blink');
    },
    grin: () => {
        face.action(3, -1, -1, 2000);
        if (actionTriggered) actionTriggered('grin');
    },
    reset: () => {
        ctx._defaultClip = 1;
        face.reset();
    ...
    sigh: () => {
        ...
        if (actionTriggered) actionTriggered('sigh');
    },
    serious: (d = 3000) => {
        ctx.play(18, THREE.LoopRepeat, 5);
        face.action(6, -1, 1, d);
        if (actionTriggered) actionTriggered('serious');
    },
    thinking: () => {
        ctx.playAction(new ActionSet(21), false);
        face.action(9, -1, 0, 1000);
        if (actionTriggered) actionTriggered('thinking');
    },
    eureka: () => {
        ctx.play(22, THREE.LoopOnce, 0);
        face.action(5, -1, 5, 1200);
        if (actionTriggered) actionTriggered('eureka');
    },
    talkAction: (index = 14) => {
        ctx.play(index);
        if (actionTriggered) actionTriggered('talkAction');
    },
    talkAction2: () => {
        face.action(3, -1, -1, 2000);
        ctx.play(15);
        if (actionTriggered) actionTriggered('talkAction2');
    },
    talkAction3: () => {
        face.action(5, -1, -1, 2000);
        ctx.play(16);
        if (actionTriggered) actionTriggered('talkAction3');
    },
    talkAction4: () => {
        face.action(4, -1, -1, 2000);
        ctx.play(17);
        if (actionTriggered) actionTriggered('talkAction4');
    }
})
...
export const showProps = (target, sceneManager, animax = undefined) => {
    ...
    const actionx = ActionProps(tomi, tomi.face, sceneManager, TOMIAnims.actionTrigger)
    tomi.defineActions(actionx);
