...
export class TOMIController extends THREE.Object3D {
  ...
  moveToRef(index = 0) {
    ...
    const cam = this.sceneManager.camera.position.clone();
    cam.y = rpos.y;
    this.moveTo(rpos, true, cam);
  }
  ...
  async bind2(mesh, scale = 1, reflect = false) {
    this.mesh = mesh;
    this.add(mesh);
  }
  ...
  async bind(mesh, scale = 1, reflect = false) {
    ...
    //this.sceneManager.scene.background = city;
    ...
    //self.updateReflect();
    // if (cubeRenderTarget) this.sceneManager.scene.environment = cubeRenderTarget.texture;
 ...
 console.log(n.morphTargetInflunces);
        if (n.material) {
        ...
        if (n.name.startsWith('Hand')) {
          const pl = new THREE.PointLight(0x00cbfe, 0.1, 0.1);
          n.add(pl);
        }
      }
      console.log(n);
  ...
  async __loadAnimations(scene, onlyClips = false, overwrite = false) {
    ...
    const mixer = new THREE.AnimationMixer(this._root);
    ...
    if (e.target.files) {
      ...
      if (res.results.length > 0) {
        res.results.forEach(scene => {
          this.sceneManager.tomi.loadTracks(scene);
        });
